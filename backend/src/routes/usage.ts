...
import { readUsageRecords, type UsageRecord, type UsageKind, type UsageUnit } from '@/lib/server/usage-storage';
...
function emptyBucket(key: string, kind: UsageKind, unit: UsageUnit): Bucket {
  return { key, kind, unit, requests: 0, inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheCreationTokens: 0, totalTokens: 0, quantity: 0 };
}
...
usageRoute.get('/', async (c) => {
  try {
    ...
    return apiError('INTERNAL_ERROR', 500, error instanceof Error ? error.message : 'Failed to read usage');
  }
});
