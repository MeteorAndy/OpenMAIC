/**
 * POST /api/extract-document — mirrors app/_api_archive/extract-document/route.ts. Public.
 * multipart/form-data via c.req.raw.formData(). Local helpers brought along verbatim.
 */
...
function supportsMimeType(provider: { supportedMimeTypes: readonly string[] }, mimeType: string): boolean {
  return provider.supportedMimeTypes.map((type) => type.toLowerCase()).includes(mimeType);
}

function isSelfHostedMinerUProvider(providerId: string): providerId is Extract<PDFProviderId, 'mineru'> {
...
export const extractDocumentRoute = new Hono();

extractDocumentRoute.post('/', async (c) => {
  ...
  try {
    ...
    if (!contentType.includes('multipart/form-data')) {
      ...
      return apiError('INVALID_REQUEST', 400, `Invalid Content-Type: expected multipart/form-data, got "${contentType}"`);
    }
    ...
    const mimeType = normalizeDocumentMimeType({ mimeType: documentFile.type, fileName: documentFile.name });
    if (!mimeType) {
      return apiError('INVALID_REQUEST', 400, `Unsupported course material type for "${documentFile.name}"`);
    }

    let provider = preferredProviderId ? getDocumentExtractorProvider(preferredProviderId) : undefined;
    ...
    try {
      ...
      return apiError('INVALID_REQUEST', 400, error instanceof Error ? error.message : `Unsupported course material type "${mimeType}"`);
    }
    ...
    const config = {
      ...
      apiKey: isPdfProviderId(provider.id) ? resolvePDFApiKey(provider.id, managed ? undefined : apiKey || undefined) : apiKey || undefined,
      baseUrl: isPdfProviderId(provider.id) ? resolvePDFBaseUrl(provider.id, clientBaseUrl) : clientBaseUrl,
    };
    ...
    log.error(`Document extraction failed [provider=${resolvedProviderId ?? 'unknown'}, file="${fileName ?? 'unknown'}"]:`, error);
