...
import { buildSearchQuery, SEARCH_QUERY_REWRITE_EXCERPT_LENGTH } from '@/lib/server/search-query-builder';
...
export const webSearchRoute = new Hono<AuthVars>();
webSearchRoute.use('*', authMiddleware);

webSearchRoute.post('/', async (c) => {
  let query: string | undefined;
  try {
    const body = await c.req.json();
    const { query: requestQuery, pdfText, providerId: requestProviderId, apiKey: bodyApiKey, baseUrl: bodyBaseUrl, baiduSubSources } = body as {
      query?: string;
      pdfText?: string;
    ...
    try {
      const { model: languageModel, thinkingConfig } = await resolveModelFromRequest(withNextUrl(c.req.raw, c.req.url), body, 'web-search-query-rewrite');
