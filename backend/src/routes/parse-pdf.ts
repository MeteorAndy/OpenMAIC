...
export const parsePdfRoute = new Hono();

parsePdfRoute.post('/', async (c) => {
  ...
  try {
    ...
    log.error(`PDF parsing failed [provider=${resolvedProviderId ?? 'unknown'}, file="${pdfFileName ?? 'unknown'}"]:`, error);
