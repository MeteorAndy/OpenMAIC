...
interface RequestBody {
  ...
  availableVoices?: Array<{ providerId: string; voiceId: string; voiceName: string; voiceLanguage?: string }>;
}
...
export const generateAgentProfilesRoute = new Hono<AuthVars>();
generateAgentProfilesRoute.use('*', authMiddleware);

generateAgentProfilesRoute.post('/', async (c) => {
  let stageName: string | undefined;
  let modelString: string | undefined;
  try {
    const body = (await c.req.json()) as RequestBody;
    const { stageInfo, sceneOutlines, languageDirective, availableAvatars, avatarDescriptions, availableVoices } = body;
    ...
    const { model: languageModel, modelString: _modelString, thinkingConfig } = await resolveModelFromRequest(
...
Return a JSON object with this exact structure:
{
  "agents": [
    ...
    const rawResult = (
      await callLLM({ model: languageModel, system: systemPrompt, prompt: userPrompt }, 'agent-profiles', undefined, thinkingConfig)
    ).text;
