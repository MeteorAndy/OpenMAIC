/**
 * POST /api/generate/scene-actions — mirrors app/_api_archive/generate/scene-actions/route.ts.
...
export const generateSceneActionsRoute = new Hono<AuthVars>();
generateSceneActionsRoute.use('*', authMiddleware);

generateSceneActionsRoute.post('/', async (c) => {
  let outlineTitle: string | undefined;
  let resolvedModelString: string | undefined;
  try {
    const body = await c.req.json();
    const { outline, allOutlines, content, stageId, agents, previousSpeeches: incomingPreviousSpeeches, userProfile, languageDirective } =
      body as {
