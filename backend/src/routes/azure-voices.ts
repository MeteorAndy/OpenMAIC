...
export const azureVoicesRoute = new Hono();

azureVoicesRoute.post('/', async (c) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return apiError('UPSTREAM_ERROR', response.status, 'Failed to fetch voices from Azure', errorText || response.statusText);
    }
    ...
    return apiError('INTERNAL_ERROR', 500, 'Failed to fetch voices', error instanceof Error ? error.message : 'Unknown error');
  }
});
