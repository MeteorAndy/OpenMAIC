...
export const quizGradeRoute = new Hono<AuthVars>();
quizGradeRoute.use('*', authMiddleware);

quizGradeRoute.post('/', async (c) => {
  ...
  try {
    ...
    const { model: languageModel, thinkingConfig } = await resolveModelFromRequest(withNextUrl(c.req.raw, c.req.url), body, 'quiz-grade');
...
${commentPrompt ? `Grading guidance: ${commentPrompt}\n` : ''}Student answer: ${userAnswer}`;
    ...
    const result = await callLLM({ model: languageModel, system: systemPrompt, prompt: userPrompt }, 'quiz-grade', undefined, thinkingConfig);
    ...
    log.error(`Quiz grading failed [question="${questionSnippet ?? 'unknown'}...", points=${resolvedPoints ?? 'unknown'}]:`, error);
