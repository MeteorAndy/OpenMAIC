...
import {
  deleteChatSessions as deleteSupabaseChatSessions,
  getChatSessions as getSupabaseChatSessions,
  replaceChatSessions as replaceSupabaseChatSessions,
...
export async function saveChatSessions(
  ...
  if (options.backend === 'supabase') {
    await replaceSupabaseChatSessions(stageId, sessions ?? []);
    return;
  }
...
export async function loadChatSessions(
  ...
  if (options.backend === 'supabase') {
    const sessions = await getSupabaseChatSessions(stageId);
    options.onSnapshot?.({ sessions: structuredClone(sessions) });
    return sessions;
  }
  const resolved = await context(options);
  const queueKey = `${stageId}\0${resolved.learnerKey}`;
...
export async function deleteChatSessions(
  stageId: string,
  options: Pick<ChatStorageOptions, 'backend'> = {},
): Promise<void> {
  if (options.backend === 'supabase') {
    await deleteSupabaseChatSessions(stageId);
    return;
  }
