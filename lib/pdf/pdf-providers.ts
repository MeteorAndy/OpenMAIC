...
function detectRasterImageMime(buffer: Buffer, contentType: string | null): string | null {
  ...
  return declared &&
    ['image/png', 'image/jpeg', 'image/gif', 'image/webp', 'image/bmp'].includes(declared)
    ? declared
    : null;
}
...
/**
 * Download an AliDocMind image URL and return a raster-image base64 data URL.
...
export async function fetchAliDocMindImageAsBase64(url: string): Promise<string | null> {
  ...
  try {
    ...
    const mime = detectRasterImageMime(buf, res.headers.get('content-type'));
    if (!mime) {
      log.warn('[AliDocMind] downloaded image has an unsupported content type');
      return null;
    }
