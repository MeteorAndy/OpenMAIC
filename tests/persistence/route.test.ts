...
describe('embedded persistence route', () => {
  ...
  it('returns a clear 404 when DATABASE_URL is unset', async () => {
    ...
    const { GET } = await import('@/app/_api_archive/persistence/[...path]/route');
  ...
  it('retries initialization on the next request after a failed pool initialization', async () => {
    ...
    const { handlePersistenceRequest } =
      await import('@/app/_api_archive/persistence/[...path]/route');
    const request = () =>
    ...
    vi.resetModules();
    const reloaded = await import('@/app/_api_archive/persistence/[...path]/route');
